...
char sct[65536]; int sctL=0; 	 // source code text
...
struct func_struct {
	uint nameL; char* nameS; uint addr, line, end;
	uint regN, regL; var_t regs[8]; 
	uint locN, locL; var_t locs[32]; 
};
...
stmt_t stmts[65536]; 	int stmtN = 0;
btrace_t bts[1024];		int btN = 0;
wpoint_t wps[1024];		int wpN = 0, wpR = 1023;
const_t cts[1024];		int ctN = 0;
int oprank[256];
		...
		if ((o_stu&255) == 0 && (stu&255) == 1) {
			...
			if (codes[i] == '(') {
				...
				if (stu&0x0800) {
					...
					(++fn)[fns] = '\0';
					if (!strcmp(fn, "enum")) {
						stu |= 0x1000; stu &= ~(0x0800); continue;
					}
					stus[stuN].name = fn;
					stus[stuN].size = 0;
					stus[stuN++].numN = 0;
		...
		if (o_stu==stu && (stu&255) == 1) {
			...
			if ((stu&0x0400) && (codes[i] == '=' || codes[i] == ',' || codes[i] == ';')) {
				...
				if (array_sz > 1) {
					...
					sprintf(func[funcN].locs[func[funcN].locN].type+strlen(func[funcN].locs[func[funcN].locN].type), "[%d]", array_sz);
					func[funcN].locN++;
					continue;
				}
			...
			if ((stu&0x0800) && (codes[i] == ',' || codes[i] == ';')) {
				...
				if (sz != -1) {
					if (array_sz > 1) {
						...
						sprintf(stus[stuN-1].nums[stus[stuN-1].numN].type+strlen(stus[stuN-1].nums[stus[stuN-1].numN].type), "[%d]", array_sz);
						stus[stuN-1].numN++;
...
void printv(int u, var_t* var) {
	uint t = 0; int i = 0, j = 0;
	if (*(var->type+strlen(var->type)-1) == ']') {
		printf("%s %s=0x%08x, ", var->type, var->name, u); return;
	}
	for ( ; i < stuN ; i++)
...
void print_watchpoint(wpoint_t* wp) {
	...
	printf("(");
	switch (wp->ltype) {
		case 0 : printf("%d", wp->lvalue); break;
		case 1 : printf("%s", ((var_t*)(((wpoint_t*)(wp->lvalue))->lvalue))->name); break;
		case 2 : print_watchpoint((wpoint_t*)(wp->lvalue)); break;
		case 3 : printf("%s:%s",
			((func_t*)(((wpoint_t*)(wp->lvalue))->rvalue))->nameS,
			((var_t*)(((wpoint_t*)(wp->lvalue))->lvalue))->name); break;
	}
	...
	switch (wp->rtype) {
		case 0 : printf("%d", wp->rvalue); break;
		case 1 : printf("%s", ((var_t*)(((wpoint_t*)(wp->rvalue))->lvalue))->name); break;
		case 2 : print_watchpoint((wpoint_t*)(wp->rvalue)); break;
		case 3 : printf("%s:%s",
			((func_t*)(((wpoint_t*)(wp->rvalue))->rvalue))->nameS,
			((var_t*)(((wpoint_t*)(wp->rvalue))->lvalue))->name); break;
	}
	printf(")");
}
...
int calc_watchpoint(wpoint_t* wp) {
	...
	char* type = NULL;
	switch (wp->ltype) {
		case 0 : lv = wp->lvalue; break;
		case 1 : 
			u = ((wpoint_t*)(wp->lvalue))->rvalue;
			type = ((var_t*)(((wpoint_t*)(wp->lvalue))->lvalue))->type;
			goto calclv;
		case 2 : lv = calc_watchpoint((wpoint_t*)(wp->lvalue)); break;
		case 3 :	
	...
	if (wp->optype == (uint)('!')) {
		...
		wp->rvalue = lv; return 0;
	}
	switch (wp->rtype) {
		case 0 : rv = wp->rvalue; break;
		case 1 : 
			u = ((wpoint_t*)(wp->rvalue))->rvalue;
			type = ((var_t*)(((wpoint_t*)(wp->rvalue))->lvalue))->type;
			goto calcrv;
		case 2 : rv = calc_watchpoint((wpoint_t*)(wp->rvalue)); break;
		case 3 :
	...
	if (lv == UNDEFINED || rv == UNDEFINED)
		return UNDEFINED;
	switch (wp->optype&255) {
		case '<' : return (wp->optype>>8) ? (lv <= rv) : (lv < rv);
		case '>' : return (wp->optype>>8) ? (lv >= rv) : (lv > rv);
		case '=' : return (lv == rv);
		case '!' : return (wp->optype>>8) ? (!lv) : (lv != rv);
		case '+' : return (lv + rv);
		case '-' : return (lv - rv);
		case '*' : return (lv * rv);
		case '/' : return (lv / rv);
...
int main(int argc, char *argv[])
{
  ...
  while (--argc && *file == '-') {
    switch(file[1]) {
    case 'g': dbg = 1; dml = *++argv; argc--; break;
